...
impl LiteralsSection {
    ...
    fn header_bytes_needed(first_byte: u8) -> Result<u8, LiteralsSectionParseError> {
    ...
    /// Parse a [LiteralsSection] from the start of `raw`, returning the parsed section
    /// and the remaining unconsumed bytes.
    pub fn parse(raw: &[u8]) -> Result<(Self, &[u8]), LiteralsSectionParseError> {
        ...
        match ls_type {
                ...
                match size_format {
                    0 | 2 => {
                        ...
                        Ok((
                            LiteralsSection {
                                ls_type,
                                regenerated_size: u32::from(raw[0]) >> 3,
                    ...
                    1 => {
                        ...
                        Ok((
                            LiteralsSection {
                                ls_type,
                                regenerated_size: (u32::from(raw[0]) >> 4)
                                    + (u32::from(raw[1]) << 4),
                                compressed_size: None,
                                num_streams: None,
                            },
            ...
            LiteralsSectionType::Compressed | LiteralsSectionType::Treeless => {
                let num_streams = match size_format {
                    0 => Some(1),
                    1..=3 => Some(4),
                    _ => panic!(
                        "This is a bug in the program. There should only be values between 0..3"
                ...
                match size_format {
                    ...
                    2 => {
                        ...
                        let regenerated_size = (u32::from(raw[0]) >> 4)
                            + (u32::from(raw[1]) << 4)
